...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const connection = await getConnection(userId, "slack", connectionId);
  if (!connection) {
    return NextResponse.redirect(new URL("/connections", _request.url), 303);
  }

  await deleteConnectionSecret(connection.secretArn);
  await deleteConnection(userId, "slack", connectionId);

  return NextResponse.redirect(new URL("/connections?disconnected=slack", _request.url), 303);
}
