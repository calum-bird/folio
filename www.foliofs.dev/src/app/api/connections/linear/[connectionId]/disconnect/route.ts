...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const connection = await getConnection(userId, "linear", connectionId);
  if (!connection) {
    return NextResponse.redirect(new URL("/connections", _request.url), 303);
  }

  await deleteConnectionSecret(connection.secretArn);
  await deleteConnection(userId, "linear", connectionId);

  return NextResponse.redirect(new URL("/connections?disconnected=linear", _request.url), 303);
}
